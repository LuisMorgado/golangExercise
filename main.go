...
func getPagination(bounderies int, currentPage int, totalPages int, around int) string {
	...
	for i := 1; i <= totalPages; i++ {
		...
		if i == currentPage || //i -> current page
			((i < currentPage && i >= currentPage-around) || (i > currentPage && i <= currentPage+around)) || // around
			(i <= bounderies || i > totalPages-bounderies) { // bounderies
			result += fmt.Sprintf("%d", i) + " "
			continue
		}

		if !earlyElipse && i > bounderies && i < currentPage { // elipse inicial
			result += "... "
			earlyElipse = true
			continue
		}

		if !lateElipse && i > currentPage && i <= totalPages-bounderies { // elipse final
			result += "... "
			lateElipse = true
			continue
		}
